<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt" exclude-result-prefixes="msxsl" xmlns:xs="http://www.w3.org/2001/XMLSchema">
    ...
    <xsl:template match="/">
        <FileSet>
            <FileSetFiles>
                <xsl:for-each select="$airtable//ProtocolTypes/ProtocolType[(normalize-space(ProtocolName) ='gwtype')]">
                ...
                <xsl:variable name="full-type-name" select="Alias"/>
                    ...
                    <FileSetFile>
                                <xsl:element name="RelativePath"><xsl:text>../../../src/gwtype/types/</xsl:text>
...
<xsl:text>
from gwtype.data_classes.</xsl:text>
<xsl:call-template name="python-case">
    <xsl:with-param name="camel-case-text" select="translate(DataClass,'.','_')"  />
</xsl:call-template>
...
<xsl:text>
from gwtype.enums import </xsl:text>
<xsl:call-template name="nt-case">
...
def check_is_market_type_name_lrd_format(v: str) -> None:
    from gwtype.enums import MarketTypeName
    try:
        x = v.split(".")
...
def check_is_market_slot_name_lrd_format(v: str) -> None:
    ...
    """
    from gwtype.data_classes.market_type import MarketType
    try:
        x = v.split(".")
    except AttributeError:
        raise ValueError(f"{v} failed to split on '.'")
